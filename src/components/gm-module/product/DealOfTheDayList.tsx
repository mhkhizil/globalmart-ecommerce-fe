...
import { useGetDealOfTheDayListInfinite } from '@/lib/hooks/service/product/useGetDealOfTheDayListInfinite';
...
function DealOfTheDayList() {
  ...
  const {
    ...
    isLoading,
    error,
  } = useGetDealOfTheDayListInfinite({
    per_page: 2,
  });
  ...
  const handleCategoryClick = useCallback(
    (categoryId: number) => {
      ...
      router.push(`/application/deal-of-the-day?${params.toString()}`);
    },
    [router]
  );
  return (
